...
class HomeVc: UIViewController, UITextFieldDelegate {
   ...
   // var tabtype = String()
    ...
    func showTables(tableCode:Int){
        tblViewSuperstar.isHidden = !(tableCode == 2 && tableCode != 1 && tableCode != 3)
        tblview.isHidden = !(tableCode != 2 && tableCode == 1 && tableCode != 3)
        brand_tblview.isHidden = !(tableCode != 2 && tableCode != 1 && tableCode == 3)
    ...
    @IBAction func languageBtnSelect(_ sender: Any) {
        let searchPostVC = storyboard?.instantiateViewController(withIdentifier: "languageVc") as! languageVc
...
//    }
        ...
        networking.MakeRequest(Url: api, Param: params, vc: self) { (result:ProfileDataModel) in
            if result.message == "success"{
                ...
                DispatchQueue.main.async {
                    print(response)
                    UserDefaults.standard.saveData(data: "\(response.first_name!) \(response.last_name!)", key: "UserName")
    ...
    private func getAdData() {
           ...
           networking.MakeRequest(Url: serverurl, Param: nil, vc: self) { (response:adResponse) in
               print(response)
               if response.message == "success"{
    ...
    fileprivate func citizentab_getfeeds(count:Int){
        ...
        let apiURL = "http://13.234.116.90/api/citizen/news?&start=\(count)&token=\(UserToken)"
        print(apiURL)
    ...
    private func superstartab_getfeeds(count:Int) {
        ...
        let apiURL = "http://13.234.116.90/api/star/news?lang_id=1&start=\(count)&token=\(UserToken)"
        print(apiURL)
        networking.MakeRequest(Url: apiURL, Param: nil, vc: self) { (result:citizenFeeds) in
            print(result)
            if result.message == "success"{
...
//                 if let feedsArray = response.data{
//                     for data in feedsArray{
//                         if !self.SellFeeds.contains(where: {$0.id == data.id}){
//                             self.SellFeeds.append(data)
//                         }
//                     }
//                     print("your array -> \(self.SellFeeds)")
//                     DispatchQueue.main.async {
//                         self.brand_tblview.reloadData()
//                     }
//                 }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if selectedTab == "1"{
            if tblview.contentOffset.y == 0{
                HeightConstraint.constant = 161
            }else{
                HeightConstraint.constant = 0
            }
        }else if selectedTab == "2"{
            if tblViewSuperstar.contentOffset.y == 0{
                      HeightConstraint.constant = 161
            }else{
                      HeightConstraint.constant = 0
            }
        }else{
            if brand_tblview.contentOffset.y == 0{
                      HeightConstraint.constant = 161
            }else{
                      HeightConstraint.constant = 0
            }
    ...
    private func infiniteScroll(scrollView:UIScrollView){
        ...
        if maximumOffset - currentOffset <= 10.0{
                ...
                if ViewFlag == "citizen" && self.Citizenfeeds.count % 10 == 0{
                    ...
                    brandlimit += 10
                    getBrandFeeds(count: brandlimit)
                }/*else if ViewFlag == "Sell" && self.SellFeeds.count % 10 == 0{
                    Selllimit += 10
                    getSellFeeds(count: Selllimit)
                }*/
