...
//        guard let postid = id else {return}
    ...
    @IBAction func btnReplyClicked(_ sender: Any) {
        ...
        //Addcomment_Citizen(postID: self.postId, replyID: replyId)
...
extension CommentVc : UITableViewDelegate,UITableViewDataSource{
        ...
        if TabType == "citizen"{
            ...
            cell.didtapedit = {
                ...
                alert.addAction(UIAlertAction(title: "Update", style: .default, handler: { (action) in
                    let comment = alert.textFields![0].text
                    if comment != nil && comment!.count > 0 {
                       self.Edit_comment(postID: self.CitizenCommentList[indexPath.row].id!, postType: "Star", newComment: comment!)
                    }
                    print("\n===================\n Edited Comment => \(comment ?? "NON") \n===================\n")
                }))
                alert.addAction(UIAlertAction(title: "Cancel", style: .destructive, handler: { (action) in
            ...
            cell.didtapedit = {
                ...
                alert.addAction(UIAlertAction(title: "Update", style: .default, handler: { (action) in
                    let comment = alert.textFields![0].text
                    if comment != nil && comment!.count > 0 {
                       self.Edit_comment(postID: self.CitizenCommentList[indexPath.row].id!, postType: "Brand", newComment: comment!)
                    }
                    print("\n===================\n Edited Comment => \(comment ?? "NON") \n===================\n")
                }))
                alert.addAction(UIAlertAction(title: "Cancel", style: .destructive, handler: { (action) in
    ...
    @objc func handleTap(_ sender: UITapGestureRecognizer) {
        if let imageView = sender.view as? UIImageView{
            if TabType == "citizen"{
                if let user_id = CitizenCommentList[imageView.tag].user_id{
                ...
                if let user_id = starCommentList[imageView.tag].user_id{
                    let story = UIStoryboard(name: "categoryStoryboard", bundle: nil)
                    let move = story.instantiateViewController(withIdentifier: "CitizenProfile") as! CitizenProfile
                    move.userId = user_id
                    navigationController?.pushViewController(move, animated: true)
                ...
                if let user_id = brandcommentList[imageView.tag].id {
                let story = UIStoryboard(name: "categoryStoryboard", bundle: nil)
                let move = story.instantiateViewController(withIdentifier: "CitizenProfile") as! CitizenProfile
                move.userId = user_id
                navigationController?.pushViewController(move, animated: true)
    ...
    fileprivate func Delete_comment(postID:String, postType: String){
        ...
        if postType == "Citizen"{
            api = "http://13.234.116.90/api/citizen/comment/delete"
        }else if postType == "Star"{
            api = "http://13.234.116.90/api/star/comment/delete"
        }else{
            api = "http://13.234.116.90/api/brands/comment/delete"
        }
           guard let UserToken = UserDefaults.standard.value(forKey: "SavedToken") as? String else {return}
           let params = ["token":UserToken,"id":postID,"comment":commenttxtview.text!] as [String:Any]
    ...
    fileprivate func Edit_comment(postID:String, postType: String,newComment: String){
     ...
     var api = ServerURL.firstpoint + ServerURL.starAddComment
     if postType == "Citizen"{
         api = "http://13.234.116.90/api/citizen/comment/edit"
     }else if postType == "Star"{
         api = "http://13.234.116.90/api/star/comment/edit"
     }else{
         api = "http://13.234.116.90/api/brands/comment/edit"
     }
        guard let UserToken = UserDefaults.standard.value(forKey: "SavedToken") as? String else {return}
