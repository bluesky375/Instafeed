...
class Language_copyVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let url: String = "http://13.234.116.90/api/locations"
...
extension Language_copyVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if setLocation {
            let url: String = "http://13.234.116.90/api/locations/set-geo"
            ...
            let url: String = "http://13.234.116.90/api/locations/get-city?state_id=\(stateId)"
            Alamofire.request(url, method: .get, encoding: URLEncoding.default).validate().responseJSON{ response in
                switch response.result {
                case .success:
                    if response.result.value != nil {
