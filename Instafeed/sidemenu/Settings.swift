...
class Settings: UIViewController {
    ...
    @IBAction func onAccount(_ sender: UIButton) {
        let move = storyboard?.instantiateViewController(withIdentifier: "AccountVCID") as! AccountVC
        navigationController?.pushViewController(move, animated: false)
    }
    
    @IBAction func onHelp(_ sender: UIButton) {
        let move = storyboard?.instantiateViewController(withIdentifier: "HelpVCID") as! HelpVC
        navigationController?.pushViewController(move, animated: false)
    }
    
    @IBAction func onAbout(_ sender: UIButton) {
        let move = storyboard?.instantiateViewController(withIdentifier: "AboutVCID") as! AboutVC
        navigationController?.pushViewController(move, animated: false)
    }
