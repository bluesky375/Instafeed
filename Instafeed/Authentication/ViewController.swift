...
//        Signin_mail_view.isHidden = false
    ...
    fileprivate func loginUser(){
        ...
        var params = ["email":email.text!, "password":password.text!, "device_id":"not now"]
...
//        }
        ...
        networking.MakeRequest(Url: api, Param: params, vc: self) { (result:ProfileDataModel) in
            if result.message == "success" {
                ...
                UserDefaults.standard.saveUserType(type: Int(response.type!)!)
            } else {
...
//        print(data)
    ...
    fileprivate func socialLogin_APIcall(provider:String, DataDict:[String:Any]){
        ...
        if (DataDict["email"] as? String) != nil{
            let email = DataDict["email"] as! String
        }

        
        let params = ["social_id":token, "provider_id":provider, "first_name":fname, "last_name":lname, "email":email ?? "", "phone":"", "password":""] as [String : Any]
        print(params)
